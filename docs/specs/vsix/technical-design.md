...
- Calculates Y-axis bounds from current visible fund returns, benchmark values, and `0%` with modest padding, so short periods such as one month do not inherit full-history extremes.
