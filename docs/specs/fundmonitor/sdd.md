...
- Calculate period returns for `1W`, `1M`, `3M`, `6M`, `1Y`, and `3Y` using the first point inside the lookback window, or the latest point before the window as fallback when no inside point exists.
