...
- The Y-axis range is calculated from current visible fund returns, benchmark values, and `0%` with modest padding, then snapped to integer, evenly spaced ticks so short periods such as `1M` stay visually compact.
