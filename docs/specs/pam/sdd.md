...
- The chart labels lowest, latest, and highest return values only when one account is shown, when an account is selected, or while an account legend item is hovered in a multi-account chart; when the highest or lowest point is also the latest point, only one value is shown.
