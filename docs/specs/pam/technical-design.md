...
- The return axis range is calculated from the current visible window plus a small padding, then snapped to integer, evenly spaced ticks so short periods such as `1M` remain compact and readable instead of inheriting full-history extremes.
