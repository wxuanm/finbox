...
- The return axis range is calculated from the current visible window and snapped to compact integer, evenly spaced ticks so short periods such as `1M` remain readable instead of inheriting full-history extremes.
