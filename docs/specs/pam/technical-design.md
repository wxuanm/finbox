...
- Lowest, latest, and highest return point markers are shown only for single-account charts, the selected account, or the legend-hovered account in multi-account charts; if the highest or lowest return value matches the latest value, only the latest value marker is shown.
...
- The return axis range is calculated from the current visible window and snapped to compact, evenly spaced ticks. Sub-1% moves may use `0.2%` or `0.5%` steps, while larger moves use integer steps so short periods such as `1M` remain readable instead of inheriting full-history extremes.
